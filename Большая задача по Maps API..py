...
pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((600, 80), (400, 100)), manager=manager, html_text='')
...
ha1i_button = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((800, 0), (100, 30)),
                                           text='Сброс',
                                           manager=manager)
hai_textbox = pygame_gui.elements.UITextEntryLine(relative_rect=pygame.Rect((600, 30), (400, 30)), manager=manager)
...
while is_running:
    response = None
    map_request = f"https://static-maps.yandex.ru/1.x/?ll={ll[0]},{ll[1]}&spn=0," + str(mashtag).replace(".",
                                                                                                         ".") + "&l=" + typ
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.__dict__['key'] == 280:
                mashtag += 1 if mashtag < 70 else 0
            if event.__dict__['key'] == 281:
                mashtag -= 1 if mashtag > 0.1 else 0
        ...
        if event.type == pygame.USEREVENT:
                    ...
                    if event.ui_element == hai_button:
                        if x(hai_textbox.text) != [1000000000.00000, 100000000.000]:
                            ...
                            try:
                                ...
                                pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((600, 80), (400, 100)),
            ...
            if event.user_type == 'ui_button_pressed':
                if event.ui_element == ha1i_button:
                    insearch = False
                    outbox = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((600, 80), (400, 100)),
                                                           manager=manager, html_text='')
                    response = requests.get(map_request)
                    map_file = "map.png"
                    if not insearch:
